...
type Args = {
  repoToken: string;
  automaticReleaseTag: string | null;
...
const createReleaseTag = async (client: Octokit, refInfo: {owner: string; repo: string; ref: string; sha: string}) => {
...
const deletePreviousGitHubRelease = async (
  client: Octokit,
  releaseInfo: {owner: string; repo: string; tag: string},
) => {
...
const searchForPreviousReleaseTag = async (
  ...
  const tagListRaw = await client.paginate(client.rest.repos.listTags, tagInfo);
  ...
  const tagList = tagListRaw
    ...
    .filter((tag: any) => tag.semverTag !== null)
    .sort((a: any, b: any) => semverRcompare(a.semverTag, b.semverTag));

  let previousReleaseTag = '';
  for (const tag of tagList) {
    if (tag.semverTag && semverLt(tag.semverTag, currentReleaseTag)) {
...
const getCommitsSinceRelease = async (
  ...
  try {
    ...
    core.info(
      `Successfully retrieved ${(resp.data as any).commits.length} commits between ${previousReleaseRef} and ${currentSha}`,
    );
  } catch (_err) {
  ...
  let commits: any[] = [];
  if (resp && resp.data && (resp.data as any).commits) {
    commits = (resp.data as any).commits;
  }
...
export const getChangelog = async (client: Octokit, owner: string, repo: string, commits: any[]): Promise<string> => {
  ...
  for (const commit of commits) {
    ...
    parsedCommitMsg.extra.pullRequests = pulls.data.map((pr: any) => {
      return {
        number: pr.number,
        url: pr.html_url,
      };
  ...
  try {
    ...
    const client = new Octokit({
      ...
      baseUrl: process.env['JEST_MOCK_HTTP_PORT']
        ...
        : undefined,
      log: {
        debug: (...logArgs: any[]) => core.debug(octokitLogger(...logArgs)),
        info: (...logArgs: any[]) => core.debug(octokitLogger(...logArgs)),
        warn: (...logArgs: any[]) => core.warning(octokitLogger(...logArgs)),
        error: (...logArgs: any[]) => core.error(octokitLogger(...logArgs)),
      },
    });
    ...
    const releaseUploadUrl = await generateNewGitHubRelease(client, {
      ...
      repo: context.repo.repo,
      tag_name: releaseTag,
      name: (args.releaseTitle ? args.releaseTitle : releaseTag) as string,
